...
There is evidence that children's propensity to help and the types of situations in which children will help changes during the second and third years of life. One-year-olds not only are less likely to help overall than two-year-olds, but also, in contrast to two-year-olds, do not help reliably in situations in which they are required to respond to a partner's emotional distress [@Svetlova2010]. One-year-olds may also require more communication from their social partner in order to help than two-year-olds, as increased requests for help and scaffolding from a social partner can also increase one-year-olds likeliness of helping [@Svetlova2010; @Spinrad2006]. Thus, two-year-olds readily help in more situations and with less guidance than one-year-olds. 
...
### Causal reasoning
[this part is very not done but the idea is to talk about causal reasoning and lead up to Hyo's study and use that as a transition into our experiments]
...
```{r, fig.cap="Schematic of toys used in Experiment 1", out.width='40%'}
...
```{r, fig.cap="Depiction of the helping phase in the Broken Toy condition. The Broken Button condition proceeded identically, except that all three toys were oriented such that the red button was on top.", out.width='40%'}
...
[all of the individual discussions are not done, because I decided to write the general discussion first and then come back to the individual ones, as I kept writing context for the individual ones that I later decided was better to talk about in the context of all experiments]

These results suggest that 2- and 3-year-olds were able to use prior knowledge to infer the likely cause of another's failure and effective help that person achieve a goal. When the likely cause of the confederate's failure was the toy itself (Broken Toy condition), children helped by changing the toy, but when the likely cause was the confederate's action on the toy (Broken Button condition), children helped by changing her action on that toy. Thus, children did not help in a uniform way across conditions; they responded to the likely cause of the confederate's failure. 
...
```{r, fig.cap="Schematic of the toys used in Experiment 2. All 4 toys were visually identical. In the Broken Button condition, the toys were also functionally identical. In the Broken Toy condition, one toy had two functional buttons and the other had two inert buttons.", out.width='40%'}
...
[I actually probably wouldn't read this discussion---it's all over the place and things got out of order because I kept rewriting sections. it currently doesn't make much sense]

In this experiment, 2- and 3-year-old children were again able to infer the likely cause of anther individual's failure and offer appropriate help. We replicated our main finding from Experiment 1---children's responses varied with the likely cause of the confederate's failure. We also replicated the finding that children in the Broken Toy condition were more likely to target a new toy than to target the confederate's toy.

In contrast to Experiment 1, children showed no preference for either toy in the Broken Button condition. In both experiments, it is possible to fulfill the confederate's goal of playing music by helping at either toy. However, the specificity of the confederate's goal is actually unclear: does she just want to play music in general (the *general* goal), or does she want to play music with her chosen toy (the *specific* goal)? If she just wants to play music and has no toy preference, than helping at either toy is equally helpful. If she wants help with her chosen toy, then it more clearly fulfills her goal to help her with that toy. However, because the toys are visually and functional identical in this condition, it is also possible to show her how her own toy works by targeting the other toy. 

We believe it is most likely that children are not distinguishing between the two toys in this condition, and this likely leads to them not attributing a toy preference to the confederate, and therefore not preferentially targeting either toy. In the Broken Toy condition, although the toys are visually identical, they are functionally distinct, which may make the distinction between the confederate's toy and the other toy more salient. This is consistent with prior research suggesting that young toddlers view a repeated choice between two objects as a preference if the two objects are visually or functionally distinct, but will not view a repeated choice between two identical objects as an expression of a side preference (CITE THINGS HERE).
...
```{r, fig.cap="Schematic of the toys used in Experiment 3. "}
...
[this is currently not organized and is mostly fast, preliminary writing that needs editing]
...
[going to insert some more general comments]

[need to change the wording of this paragraph] There are two broad questions about how to categorize children's behavior in these experiments. First, are children really helping the confederate, or are they merely acting to produce music for themselves? Second, are children helping the confederate or are they informing her?
...
three_toy %>% 
  select(condition, contains("first"), contains("second")) %>% 
  filter(firstChoice == secondChoice,
         str_detect(secondBehaviorCode, "flip"),
         condition == "person")
...
- connection between false belief understanding and informing---would we see more informing behavior with older children who are reliably able to represent and reason about false beliefs?
