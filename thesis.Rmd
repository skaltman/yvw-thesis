...
goal attribution
- meltzoff 1995
  - at 18 months cna figure out goal from actions even if those actions are incomplete
...
```{r}
# this is messy--fix
# does age predict choice when you control for condition?
# no
glm(firstChoice ~ age + condition, family = "binomial", 
    data = 
      two_toy_tidy %>% 
...
# flip
# no
glm(flip ~ age, family = "binomial", data = two_toy_tidy ) %>% 
  summary()

# no
glm(flip ~ age, family = "binomial", data = modified_tidy ) %>% 
  summary()
...
# no
glm(helpfulCategory ~ age + condition, family = "binomial", 
    data = 
      three_toy_tidy %>% 
      mutate(helpfulCategory = helpfulCategory == "Helpful")) %>% 
  summary()
...
#flip
glm(flip ~ age, family = "binomial", data = three_toy_tidy) %>% 
  summary()

#correctness

# two toy
# no age does not predict
glm(firstChoiceCorrect ~ age, family = "binomial", data = two_toy_tidy) %>% 
  summary()
